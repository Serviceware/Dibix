...
namespace Dibix.Sdk
{
    internal abstract class PackageLocator
    {
        ...
        public string LocatePackage(string packageName)
        {
            ...
            {
                string libDirectory = Path.Combine(packageDirectory, LibDirectoryName);
                packagePath = Directory.EnumerateFiles(libDirectory, packageFileName, SearchOption.AllDirectories)
                                       .FirstOrDefault();
            }
...
{Path.Combine(absolutePackageSearchDirectory, searchPattern, LibDirectoryName, packageFileName)}");
