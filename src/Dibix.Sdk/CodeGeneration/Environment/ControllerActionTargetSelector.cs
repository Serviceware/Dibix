...
namespace Dibix.Sdk.CodeGeneration
{
    internal sealed class ControllerActionTargetSelector : IControllerActionTargetSelector
    {
        ...
        public ControllerActionTargetSelector(string productName, string areaName, string outputName, string projectDirectory, ICollection<SqlStatementInfo> statements, IEnumerable<string> references, IErrorReporter errorReporter)
        {
            ...
            this._neighborStatementMapAccessor = new Lazy<IDictionary<string, string>>(() => CreateNeighborStatementMap(projectDirectory, references));
        }
        ...
        public ActionDefinitionTarget Select(string target, string filePath, IJsonLineInfo lineInfo)
        {
            ...
            if (!isAbsolute)
                normalizedNamespace = NamespaceUtility.BuildNamespace(this._productName, this._areaName, LayerName.Data, @namespace);
            ...
            bool isLocal = this._statements.Any(x => x.Namespace.FullNamespace == normalizedNamespace && x.Name == methodName);
            if (isLocal)
...
Tried: {normalizedNamespace}.{methodName}");
        ...
        private static IDictionary<string, string> CreateNeighborStatementMap(string projectDirectory, IEnumerable<string> references) => CreateNeighborStatementMapCore(projectDirectory, references).ToDictionary(x => x.Key, x => x.Value);

        private static IEnumerable<KeyValuePair<string, string>> CreateNeighborStatementMapCore(string projectDirectory, IEnumerable<string> references)
        {
            return from reference in references 
                   select Path.GetFullPath(Path.Combine(projectDirectory, reference)) into fullPath 
                   select Assembly.ReflectionOnlyLoadFrom(fullPath) into assembly 
