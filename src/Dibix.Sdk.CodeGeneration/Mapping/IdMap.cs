...
namespace Dibix.Sdk.CodeGeneration
{
    internal sealed class IdMap<TFlag>
    {
        ...
        private IdMap(Type type) => _type = type;

        public static IdMap<TFlag> Create(Type type) => new IdMap<TFlag>(type);

        public string GetDefinitionName(TFlag flag) => FlagCache.GetFlagName(_type, flag);

        public bool TryGetDefinitionName(TFlag flag, out string definitionName) => FlagCache.TryGetFlagName(_type, flag, out definitionName);
    }
